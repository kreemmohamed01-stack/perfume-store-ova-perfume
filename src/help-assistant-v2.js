(function () {
  ...
  function init() {
    ...
    style.textContent = `
      ...
      .ova-help-float::before{content:"";display:block;width:68px;height:68px;flex:0 0 auto;background:center/contain no-repeat url("images/ai-agent/agent-icon.webp");filter:drop-shadow(0 6px 14px rgba(20,18,16,.34)) drop-shadow(0 2px 4px rgba(20,18,16,.22));animation:ovaAiZoom 3.4s ease-in-out infinite;transform-origin:center;overflow:visible;clip-path:none;-webkit-clip-path:none}
      @keyframes ovaAiZoom{0%,100%{transform:scale(1)}50%{transform:scale(1.07)}}
      .ova-help-float::after{content:attr(data-label);display:block;font:700 8px/1 'Manrope',sans-serif;letter-spacing:.11em;text-transform:uppercase;color:#1f1a14;white-space:nowrap}
